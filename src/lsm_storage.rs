...
impl LsmStorageInner {
    ...
    pub fn delete(&self, key: &[u8]) -> Result<()> {
        // 1. Get the current memtable.
        let state = self.state.read();

        // 2. Write the deletion tombstone.
        state.memtable.put(key, b"")?;

        // 3. Fast path: memtable is still below the limit.
        if state.memtable.approximate_size() < self.options.target_sst_size {
            return Ok(());
        }

        // 4. Release the read lock before acquiring state_lock.
        drop(state);

        // 5. Serialize freeze operations.
        let state_lock = self.state_lock.lock();

        // 6. Re-read the current state.
        let state = self.state.read();

        // 7. Another thread may already have frozen the memtable.
        if state.memtable.approximate_size() < self.options.target_sst_size {
            return Ok(());
        }
        ...
        // 9. Freeze the still-full memtable.
        self.force_freeze_memtable(&state_lock)?;
